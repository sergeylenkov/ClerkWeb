...
module.exports.getBalance = () => {
  return new Promise((resolve, reject) => {
    db.all(`SELECT a.id, a.name, a.credit_limit, c.short_name AS currency_name, a.type_id,
                ...
                FROM accounts a, currencies c WHERE (a.type_id = 1 OR a.type_id = 5) AND a.active = 1 AND a.currency_id = c.id ORDER BY a.order_id`, [], (err, rows) => {
      if (err) {
        reject(err);
      } else {
        let items = [];
...
module.exports.getReceipts = (from, to) => {
  return new Promise((resolve, reject) => {
    db.all(`SELECT a.id, a.name, TOTAL(t.from_account_amount) as sum FROM transactions t, accounts a
             WHERE a.type_id = 0 AND t.from_account_id = a.id AND t.paid_at >= ? AND t.paid_at <= ? AND t.deleted = 0 GROUP BY a.name ORDER BY sum DESC`, [from, to], (err, rows) => {
      if (err) {
        reject(err);
      } else {
        let items = [];
...
module.exports.getSchedulers = function (from, to) {
  return new Promise((resolve, reject) => {
    db.all(`SELECT s.id, s.name, s.from_account_amount, s.to_account_amount, s.next_date FROM schedulers s
             WHERE s.next_date >= ? AND s.next_date <= ? AND s.active = 1 ORDER BY s.id`, [from, to], (err, rows) => {
      if (err) {
        reject(err);
      } else {
        let items = [];
...
function _getBudgetBalance(ids, from, to) {
  return new Promise((resolve, reject) => {
    db.get(`SELECT TOTAL(t.to_account_amount) AS sum FROM transactions t, accounts a
            WHERE a.type_id = 2 AND t.to_account_id IN(${ids}) AND t.to_account_id = a.id AND t.paid_at >= ? AND t.paid_at <= ? AND t.deleted = 0`, [from, to], (err, row) => {
      if (err) {
        reject(err);
      } else {
        resolve(row.sum);
