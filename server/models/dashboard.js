...
module.exports.getBalance = function() {
    return new Promise((resolve, reject) => {
        db.all('SELECT a.id, a.name, c.short_name AS currency_name,\
            ...
            if (err) {
                ...
                rows.forEach((row) => {
                    let item = { id: row.id, name: row.name, receipt: row.receipt, expense: row.expense, currency: row.currency_name };
                    items.push(item);
                });
