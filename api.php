...
//$mysql = new PDO("mysql:host=127.0.0.1;dbname=clerk;charset=utf8", "mysql", "mysql");
...
if ($_GET["action"] == "budget") {
    $receipt = $mysql->query("SELECT COALESCE(SUM(t.to_account_amount), 0) AS sum FROM accounts a, transactions t WHERE a.type_id = 0 AND t.from_account_id = a.id AND t.deleted = 0 AND t.paid_at >= '" . $_GET["from"] . "' AND t.paid_at <= '" . $_GET["to"] . "'")->fetch();
    $expense = $mysql->query("SELECT COALESCE(SUM(t.from_account_amount), 0) AS sum FROM accounts a, transactions t WHERE a.type_id = 2 AND t.to_account_id = a.id AND t.deleted = 0 AND t.paid_at >= '" . $_GET["from"] . "' AND t.paid_at <= '" . $_GET["to"] . "'")->fetch();
...
if ($_GET["action"] == "expenses") {
    ...
    foreach ($mysql->query("SELECT a.name AS name, SUM(t.from_account_amount) AS sum FROM accounts a, transactions t WHERE a.type_id = 2 AND t.to_account_id = a.id AND t.deleted = 0 AND t.paid_at >= '" . $_GET["from"] . "' AND t.paid_at <= '" . $_GET["to"] . "' GROUP BY t.to_account_id ORDER BY sum DESC LIMIT 5") as $row) {
...
if ($_GET["action"] == "last_transactions") {
    ...
    foreach ($mysql->query("SELECT a1.name AS from_account_name, a1.type_id AS from_type_id, a2.name AS to_account_name, a2.type_id AS to_type_id, t.* FROM transactions t, accounts a1, accounts a2
        ...
        foreach ($mysql->query("SELECT t.name FROM transactions_tags tt, tags t WHERE tt.transaction_id = " . $row["id"] . " AND t.id = tt.tag_id") as $row2) {
...
if ($_GET["action"] == "transactions") {
    ...
    foreach ($mysql->query("SELECT a1.name AS from_account_name, a1.id AS from_account_id, a1.type_id AS from_type_id, a2.name AS to_account_name, a2.id AS to_account_id, a2.type_id AS to_type_id, t.* FROM transactions t, accounts a1, accounts a2
...
if ($_GET["action"] == "tags") {
	...
	foreach ($mysql->query("SELECT t.name FROM transactions_tags tt, tags t WHERE tt.transaction_id = " . $_GET["id"] . " AND t.id = tt.tag_id") as $row) {
...
if ($_GET["action"] == "transaction") {
    if ($_GET["mode"] == "insert") {
        $error = false;
        $statement = $mysql->prepare("INSERT INTO transactions (paid_at, from_account_id, to_account_id, from_account_amount, to_account_amount, note, created_at, deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($statement->execute(array($_GET['date'], $_GET['from_account'], $_GET['to_account'], $_GET['from_amount'], $_GET['to_amount'], $note, $date, 0))) {
            $transaction_id = $mysql->lastInsertId();
            $tags = explode(",", $_GET['tags']);

            foreach ($tags as $tag) {
                $tag_id = -1;
                $result = $mysql->query("SELECT id FROM tags WHERE name = '" . trim($tag) . "'")->fetch();
                ...
                if ($tag_id == -1) {
                    $statement = $mysql->prepare("INSERT INTO tags (name) VALUES (?)");

                    if ($statement->execute(array(trim($tag)))) {
                        $tag_id = $mysql->lastInsertId();
                    }
                }

                $statement = $mysql->prepare("INSERT INTO transactions_tags (tag_id, transaction_id) VALUES (?, ?)");
        ...
        $error = false;
        $statement = $mysql->prepare("UPDATE transactions SET paid_at = ?, from_account_id = ?, to_account_id = ?, from_account_amount = ?, to_account_amount = ?, note = ? WHERE id = ?");
        ...
        if ($statement->execute(array($_GET['date'], $_GET['from_account'], $_GET['to_account'], $_GET['from_amount'], $_GET['to_amount'], $note, $transaction_id))) {
            $statement = $mysql->prepare("DELETE FROM transactions_tags WHERE transaction_id = ?");
        ...
        $row = $mysql->query("SELECT a1.name AS from_account_name, a1.type_id AS from_type_id, a2.name AS to_account_name, a2.type_id AS to_type_id, t.* FROM transactions t, accounts a1, accounts a2 WHERE t.id = " . $_GET["id"] . " AND a1.id = t.from_account_id AND a2.id = t.to_account_id ORDER BY paid_at DESC, created_at DESC")->fetch();
        ...
        $statement = $mysql->prepare("UPDATE transactions SET deleted = ? WHERE id = ?");
...
if ($_GET["action"] == "all_tags") {
	...
	foreach ($mysql->query("SELECT DISTINCT(TRIM(t.name)) as name FROM tags t") as $row) {
        $tags[] = array("name" => $row["name"]);
    }
